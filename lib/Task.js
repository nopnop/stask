...
// Max length for a label in a filename (to be both usable and explicit)
...
class Task {
   ...
   * @return {String}
   */
  getExpectedFilepath (options) {
    options = Object.assign({
      dir: process.cwd()
    }, options || {})
    let newFilename = this.id + '_' + kebabcase(this.label).slice(0, MAX_LABEL_LEN) + '.md'
    let relativeTo = this.filepath ? path.dirname(this.filepath) : options.dir
  ...
  write (options) {
    return co(function * () {
      ...
      let newFilepath = this.getExpectedFilepath(options)
   ...
   */
  unlink () {
    debug('unlink: %s', this.id)
    return fs.unlinkAsync(this.filepath)
  }

  /**
   * Move to a .trash folder (safer than unlink)
   */
  trash () {
    debug('trash: %s', this.id)
    return fs.moveAsync(this.filepath, path.join(path.dirname(this.filepath), '.trash', Date.now() + '-' + path.basename(this.filepath)))
  }
